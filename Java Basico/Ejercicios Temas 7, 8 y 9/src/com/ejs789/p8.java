package com.ejs789;

import java.io.FileInputStream;
import java.io.InputStream;
import java.io.PrintStream;
import java.util.Scanner;

/*
 * Consigna del ejercicio:
 * Utilizando InputStream y PrintStream, crea una función que reciba dos parámetros: 
 * "fileIn" y "fileOut". La tarea de la función será realizar la copia del fichero dado
 * en el parámetro "fileIn" al fichero dado en "fileOut".
 */


public class p8 {

    static void copiarFichero(String fileIn, String fileOut) throws Exception {

        try{
            InputStream in = new FileInputStream(fileIn);
            byte[] datos = in.readAllBytes();
            in.close();

            PrintStream out = new PrintStream(fileOut);
            out.write(datos);
            out.close();

        } catch (Exception e) {
            System.out.println("Se ha encontrado una excepcion: " + e.getMessage());
        }
    }


    public static void main(String[] args) throws Exception {

        Scanner scanner = new Scanner(System.in);

        System.out.println("Igrese el fichero de origen: ");
        String fileIn = scanner.nextLine();

        System.out.println("Ingrese el fichero de destino: ");
        String fileOut = scanner.nextLine();

        copiarFichero(fileIn, fileOut);

        scanner.close();
    }
}
